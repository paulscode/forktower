...
func TestNewRejectsAnUnusableConfiguration(t *testing.T) {
	...
	if _, err := New(h.store, h.sen, h.alerter, h.ln, h.dl, h.wa, h.sd, Config{Auth: config.AuthPassword}, nil, nil); err == nil {
		t.Error("password mode was accepted with no password set, which nobody could ever sign in to")
	}
	if _, err := New(h.store, h.sen, h.alerter, h.ln, h.dl, h.wa, h.sd, Config{Auth: "something-else"}, nil, nil); err == nil {
		t.Error("an unknown authentication mode was accepted")
	}
	if _, err := New(nil, h.sen, h.alerter, h.ln, h.dl, h.wa, h.sd, Config{}, nil, nil); err == nil {
		t.Error("a server with no store was accepted")
	}
	if _, err := New(h.store, nil, h.alerter, h.ln, h.dl, h.wa, h.sd, Config{}, nil, nil); err == nil {
