...
export const configJson = FileHelper.json(
  ...
  z.object({
    ...
    // The companion tower's own log level, separate from Forktower's.
    //
    // **Separate because it was needed once and could not be had.** Working out
    // why no watchtower session was ever agreed came down to whether the client's
    // connection reached the tower, and the tower's `lnd.conf` is regenerated by
    // the entrypoint on every start — so raising it meant editing a file that
    // would be overwritten, or a rebuild. The client end could be raised at
    // runtime; the tower end could not.
    towerLogLevel: z.enum(['info', 'debug', 'trace']).catch('info'),
  }),
)
