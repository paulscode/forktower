...
# enforces the new rules that is the chain their node cannot see.
#
# Written here rather than baked into the image because the RPC password is
# generated per installation, and because the listen address is a decision the
# platform makes.
write_tower_conf() {
  tower_dir="${DATA_DIR}/tower"
  conf="${tower_dir}/lnd.conf"
  mkdir -p "${tower_dir}"

  if [ -f "${conf}" ] && ! head -n1 "${conf}" | grep -qF 'generated by'; then
    log "keeping the tower lnd.conf you wrote at ${conf}"
    return
  fi
...
# A watchtower and nothing else. No channels are opened here, no payments are
# routed, and the wallet exists only because lnd insists on one.
[Application Options]
lnddir=${tower_dir}
# No seed to write down, because there is nothing here worth recovering. The
# tower holds encrypted blobs belonging to somebody else's node.
noseedbackup=true
...
rpclisten=127.0.0.1:${FORKTOWER_TOWER_LND_RPC_PORT:-10019}
debuglevel=${FORKTOWER_TOWER_LND_LOG_LEVEL:-info}

[Bitcoin]
bitcoin.node=bitcoind
bitcoin.${FORKTOWER_TOWER_LND_NETWORK:-mainnet}=true

# **The second node, not the user's own.** Watching their chain would make this
# tower a duplicate of the protection they already have, and leave the chain
# that actually needs watching unwatched.
[Bitcoind]
bitcoind.rpchost=${SQ_RPC_HOST:-127.0.0.1:8432}
bitcoind.rpcuser=forktower
bitcoind.rpcpass=${FORKTOWER_SQ_RPC_PASSWORD}
bitcoind.zmqpubrawblock=tcp://127.0.0.1:${FORKTOWER_SQ_ZMQ_BLOCK_PORT:-28432}
bitcoind.zmqpubrawtx=tcp://127.0.0.1:${FORKTOWER_SQ_ZMQ_TX_PORT:-28433}
...
CONF
    ...
    # onion on StartOS, a LAN address on Umbrel — it has to be advertised, or
    # the URI the dashboard shows is one nobody outside this container can
    # reach.
    if [ -n "${FORKTOWER_TOWER_LND_EXTERNAL_ADDR:-}" ]; then
      printf 'watchtower.externalip=%s\n' "${FORKTOWER_TOWER_LND_EXTERNAL_ADDR}"
    fi
  } > "${conf}"
